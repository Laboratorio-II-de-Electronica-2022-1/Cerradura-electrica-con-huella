...
void setup() {
  ...
  RtcDateTime inicial = rtc.GetDateTime();
  String FechaIni = String(inicial.Day())+"/"+String(inicial.Month())+"/"+String(inicial.Year())+" "+String(inicial.Hour())+":"+String(inicial.Minute())+":"+String(inicial.Second());
...
void loop() {
  ...
  String Fecha = String(now.Day())+"/"+String(now.Month())+"/"+String(now.Year())+" "+String(now.Hour())+":"+String(now.Minute())+":"+String(now.Second());
...
////////////////////////////////Scan button pressed/////////////////////////////////////////////////
 ...
 while(scanning && counter <= 60)
 {
  ...
  if(counter == 59)
  {
    ...
    lcd.print("   Try again!   ");
    myFile = SD.open(file_name_to_save, FILE_WRITE);
    myFile.print(Fecha);// myFile.print(" -- "); myFile.print(rtc.getTimeStr());
...
if(digitalRead(scan_pin) && id_ad)
 {  
  ...
  id=num;
  while (!  getFingerprintEnroll(Fecha) );
  id_ad = false;
  lcd.clear();
  lcd.setCursor(0,0);
...
uint8_t getFingerprintID(String Fecha)
{
...
uint8_t getFingerprintEnroll(String Fecha) {
 ...
 lcd.print(id);
  ...
  if (p == FINGERPRINT_OK) {
    ...
    lcd.print("                ");
    myFile = SD.open(file_name_to_save, FILE_WRITE);
    myFile.print(Fecha);// myFile.print(" -- "); myFile.print(rtc.getTime());
